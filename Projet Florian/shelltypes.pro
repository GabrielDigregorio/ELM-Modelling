...
DefineConstant[
  ...
  R_out_value = {5.7595, Min 1e-9, Max 1e9, Step 1,
    Name "Parameters/Output_Resistance"}
	
  conductivity_value = {5.69e7, Min 10^5, Max 7e8, Step 10,
    Name "Parameters/Conductivity"}	

  mur_value = {2000, Min 1, Max 3000, Step 10,
    Name "Parameters/mur_value"}	
];

Group {
  // Physical regions:
  AIR_WINDOW = Region[{AIR_WINDOWL,AIR_WINDOWR}];
...
PostOperation {
  { Name Map_a; NameOfPostProcessing Magnetodynamics2D_av;
    Operation {
      // Print[ j,OnElementsOf Coil_2/*Region[{Vol_C_Mag, Vol_S_Mag}]*/, Format Gmsh, File "j.pos"];//
	  ...
	  // Print[ U, OnElementsOf Coil_1, File "U.pos"];
	  // Print[ U, OnElementsOf Coil_2, File "U2.pos"];
	  // Print[ j, OnElementsOf Coil_1, Format Gmsh, File "j1.pos"];
	  // Print[ j, OnElementsOf Coil_2, Format Gmsh, File "j2.pos"];
    }
  }
}
