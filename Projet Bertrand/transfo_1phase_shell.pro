...
Include "transfo_common_1phase_shell.pro";
...
DefineConstant[
  ...
  R_out_value = {24/4.1667, Min 0, Max 10^6, Step 100,
    Name "Parameters/R_out"}
  Wire_cond = {59.6*10^6, Min 1000, Max 10^10, Step 10000,
    Name "Parameters/Conductivity"}
  Core_perm = {2000 , Min 100, Max 5100, Step 500,
    Name "Parameters/permeability"}
  Cond_core = {10^7 , Min 1, Max 10^7, Step 1000,
    Name "Parameters/Cond_core"}
];

Group {
  // Physical regions:
  Air = Region[{Air_window_right,Air_window_left, Air_ext}];
  ...
  Vol_Mag = Region[{Air, Core, Coils/*,Shield*/}]; // full magnetic domain
  If (type_Conds == 1)
    Vol_C_Mag = Region[{Core,Coils}]; // massive conductors
  ElseIf (type_Conds == 2)
...
Function {
  ...
  //mu[Shield] = mu0*mur_Shield;
  
  mu[Coils] = 1 * mu0;
  sigma[Coils] = Wire_cond;
  sigma[Core] = Cond_core;
...
PostOperation {	
  { Name Map_a; NameOfPostProcessing Magnetodynamics2D_av;
    Operation {
	  ...
	  //Print[ JouleLosses[Vol_Mag], OnGlobal, Format Table, File > "joule_losses.txt"];
