...
Line Loop(firstlineloop) = {firstline:last_line};
...
firstline=newreg;
Line(firstline) = {1, Hpoint}; // Domain
Line(newreg) = { Hpoint,2};
Line(newreg) = {2,Vpoint};
break2=newreg;
Line(break2) = {Vpoint,3};
Line(newreg) = {3, 4};
Line(newreg) = {4, 1};
break1=newreg;
Line(break1) = {Hpoint, middlPoint};
Line(newreg) = {middlPoint, Vpoint};

firstlineloop=newreg;
Line Loop(firstlineloop) = {firstline,break1,break1+1,break2,break2+1,break2+2};
secondlineloop=newreg;
Line Loop(secondlineloop) = {firstline+1,firstline+2,-(break1+1),-break1};
Plane Surface(2)={secondlineloop};
...
Physical Surface("Air", 101) = 1;  /* Air */
Physical Line("Wire", 102) = {stock_circle[]};  /* Wire */
Physical Line("Domain", 103) = {firstline,break1,break1+1,break2,break2+1,break2+2};  /* Domain */
Printf("firstlineloop = '%g' ",firstlineloop);

//Physical Line("Box", 103) = {};  /* Box = Ind */

//Physical Line("AirInf", 111) = {};  /* AirInf */
